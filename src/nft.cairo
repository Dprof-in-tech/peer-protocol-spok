    ...
    impl IMintableImpl of super::IMintable<ContractState> {
        fn mint(ref self: ContractState,reciever_Address: ContractAddress, unique_id: u256, proposal_id: u256) {
            ...
            self.latest_token_id.write(token_id);
             ...
             let protocol = self.peer_protocol_address.read();

             let token_id_u256: u256 = token_id.into();

             //assert(caller == protocol, 'Only protocol can mint');
            ...
            self.erc721.mint(minter, token_id_u256);

            self.erc721.set_approval_for_all(protocol, true);
            self.erc721.set_approval_for_all(caller, true);
        }
